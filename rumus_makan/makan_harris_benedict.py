...
class makan_harrisbenedict():
    def __init__(self, bb=None, tb=None, usia=None, sex=None, aktivitas=None):
        self.bb = bb 
        self.tb = tb 
        self.usia = usia 
        self.sex = sex
    ...
    def EnergiPagi(self):
        if self.sex == 'wanita':
            bmr = 655 + (9.6 * self.bb) + (1.8 * sobjectelf.tb) - (4.7 * self.usia)
        
        elif self.sex == 'pria':
            bmr = 66 + (13.7 * self.bb) + (5 * self.tb) - (6.8 * self.usia)
        
        tee = bmr * self.aktivitas 
